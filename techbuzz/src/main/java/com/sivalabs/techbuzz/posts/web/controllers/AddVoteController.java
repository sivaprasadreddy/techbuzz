...
class AddVoteController {
    ...
    @ResponseBody
    public String createVoteHTMX(
            @PathVariable String postId, @PathVariable String voteType, @CurrentUser User loginUser) {
        CreateVoteRequest createVoteRequest =
                new CreateVoteRequest(Long.valueOf(postId), loginUser.getId(), Integer.valueOf(voteType));
        postService.addVote(createVoteRequest);
        ...
        returnHtml.append(
                "<button class=\"btn btn-success" + (Integer.valueOf(voteType) == 1 ? " disabled" : "") + "\"");
        ...
        returnHtml.append(
                "<button class=\"btn btn-danger" + (Integer.valueOf(voteType) == -1 ? " disabled" : "") + "\"");
