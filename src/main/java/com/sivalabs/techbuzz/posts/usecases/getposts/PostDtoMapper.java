...
public class PostDtoMapper {
	...
	public PostDTO toDTO(User loginUser, Post post) {
		...
		if (votes != null) {
			voteDTOS = votes.stream().map(v -> new VoteDTO(v.getId(), v.getUserId(), v.getPostId(), v.getValue()))
					.collect(Collectors.toSet());

			if (loginUser != null) {
				upVoted = votes.stream()
						.anyMatch(v -> Objects.equals(v.getUserId(), loginUser.getId()) && v.getValue() == 1);
				downVoted = votes.stream()
						.anyMatch(v -> Objects.equals(v.getUserId(), loginUser.getId()) && v.getValue() == -1);
			}
		}
		return new PostDTO(post.getId(), post.getTitle(), post.getUrl(), post.getContent(), category, voteDTOS,
				post.getCreatedBy().getId(), post.getCreatedBy().getName(), post.getCreatedAt(), post.getUpdatedAt(),
				editable, upVoted, downVoted);
	}
