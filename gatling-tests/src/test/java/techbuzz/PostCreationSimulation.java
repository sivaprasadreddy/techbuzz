...
public class PostCreationSimulation extends Simulation {
    ...
    ChainBuilder createPostFlow =
            exec(login)
            .pause(2)
            .exec(createPost);

    ScenarioBuilder scnCreatePost = scenario("Create Post")
    ...
    {
        setUp(
            scnCreatePost.injectOpen(rampUsers(getConfig().getInt("users")).during(10))
        )
